    ...
    public class QuadBatch {
        ...
        public void End()
        {
            ...
            if (vert_count >= 3) {
                ...
                default_shader.Parameters["MatrixTransform"].SetValue(WorldViewProj); // if this line fails, check the name of the transform in the QuadBatch.fx
        ...
        public void DrawString(string text, Vector2 pos, Vector2 scale, Color color, Alignment align = Alignment.None)
        {
            ...
            do {
                ...
                fontverts[font_vert_count].Color = color; font_vert_count++;
                if ((font_vert_count + 1) >= 8180) { //Flush 
        ...
        #region A D D   Q U A D   (helper for DrawChained stuff)
        ...
        #region A D D   Q U A D   (helper for DrawChained stuff)
        // For programming convenience, this separate method can be used to make code look more compact. If optimizing becomes necessary, maybe better to do directly. 
        void AddQuad(Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4, float u1, float v1, float u2, float v2, Color color)
        {
            vertices[vert_count].Position = new Vector3(p1.X, p1.Y, depth); vertices[vert_count].TextureCoordinate = new Vector2(u1, v1);   //upper-left texCoord
            vertices[vert_count].Color = color; vert_count++;
            vertices[vert_count].Position = new Vector3(p2.X, p2.Y, depth); vertices[vert_count].TextureCoordinate = new Vector2(u2, v1);   //upper-right texCoord
            vertices[vert_count].Color = color; vert_count++;
            vertices[vert_count].Position = new Vector3(p3.X, p3.Y, depth); vertices[vert_count].TextureCoordinate = new Vector2(u2, v2);   //lower-right texCoord
            vertices[vert_count].Color = color; vert_count++;
            vertices[vert_count].Position = new Vector3(p4.X, p4.Y, depth); vertices[vert_count].TextureCoordinate = new Vector2(u1, v2);   //lower-left texCoord
            vertices[vert_count].Color = color; vert_count++;
            if ((vert_count + 1) >= 8180) {
                EndVerts(); beginCalled = true; vert_count = 0;
            }
        ...
        // D R A W   T A I L 
        // NOTE: requires that source image is horizontal on sprite sheet (could make another for vertical too)           //const bool show_wires = true;
        public void DrawTail(Rectangle sourceRect, Color color, Vector2 pos, TailBone[] bones, float scale = 1f, SpriteEffects flip = SpriteEffects.None)       
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before QuadBatch Draw. Draw aborted."); return; }
            int cnt = bones.Length;
            ...
            // create the initial mesh points:
            Vector2 p1 = pos, p2 = bones[0].pos, ofu = Vector2.Zero, ofd = Vector2.Zero, ovu = Vector2.Zero, ovd = Vector2.Zero;
            Vector2 fu = Vector2.Zero, fd = Vector2.Zero, au = Vector2.Zero, ad= Vector2.Zero;
            u1 = u2 - uv_div;
            for(int i=0; i<cnt; i++) {
                if (i!=0) { p1 = bones[i-1].pos; p2 = bones[i].pos; }                                           // if (show_wires) Line(p1.X, p1.Y, p2.X, p2.Y, Color.Red);
                Vector2 j = p2 - p1;                       // get joint vector
                Vector2 up = new Vector2(-j.Y, j.X);       // get upward vector
                Vector2 n = Vector2.Normalize(up);         // length as 1
                up = n * hh;                               // length as half-height
                Vector2 vu = p1 + up;                      // vector position at top        
                Vector2 vd = p1 - up;                      // vector position at bottom                        //if (show_wires) Line(p1.X, p1.Y, vu.X, vu.Y, Color.Green);  if (show_wires) Line(p1.X, p1.Y, vd.X, vd.Y, Color.Teal);
                fu = vu + j;                               // far-up (left is vec-up + joint-vec) 
                fd = vd + j;                                                                                   //if (show_wires) Line(p2.X, p2.Y, fu.X, fu.Y, Color.Blue);   if (show_wires) Line(p2.X, p2.Y, fd.X, fd.Y, Color.Purple);
                au = vu; ad = vd;
                if (i>0) {
                    au += ofu; ad += ofd;                  // get average of the 2 up vectors (now and before) and add quad using finished vectors 
                    au *= 0.5f; ad *= 0.5f;
                    AddQuad(au, ovu, ovd, ad, u1, v1, u2, v2, color);
                    u1 -= uv_div; u2 -= uv_div;            // adjust uv sample source (move left a bit on the image)
                }                                                                                             //if (show_wires) Line(p1.X, p1.Y, au.X, au.Y, Color.Yellow);   if (show_wires) Line(p1.X, p1.Y, ad.X, ad.Y, Color.Orange);                
                ofu = fu; ofd = fd; ovu = au; ovd = ad;
            }
            AddQuad(fu, au, ad, fd, u1, v1, u2, v2, color);
        }//Draw (Non-Uniform)
