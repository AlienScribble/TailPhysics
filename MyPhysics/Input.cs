...
namespace AlienScribble {
    class Input {
        ...
        public Vector2 mosV, omosV;
        public int mosx, mosy;
        public Point mp;

        // GAMEPAD STUFF   Note: Triggers are variable (0.0f to 1.0f)
        ...
        {
            screenScaleX = 1.0f / ((float)pp.BackBufferWidth / (float)target.Width);   // used with mouse coord calculations
            screenScaleY = 1.0f / ((float)pp.BackBufferHeight / (float)target.Height);
            // SET PLAYER 1 DEFAULT KEYS             
        }
        ...
        public void Update()
        {
            ...
            okb = kb; oms = ms; ogp = gp; omosV = mosV;
        ...
        //// E S C A P E  P R E S S
        public bool Escape()
        {
            if ((kb.IsKeyDown(Keys.Escape) && okb.IsKeyUp(Keys.Escape))) return true;
            return false;
        }
        ...
        //public bool EscapePress() //prevents escape input from happening more than once in a loop (like if exiting menu or sub menu)
        //{
        //    if (allow_escape) {
        //        if ((kb.IsKeyDown(Keys.Escape) && okb.IsKeyUp(Keys.Escape))) { allow_escape = false; return true; }
        //        else return false;
        //    }
        //    else {
        //        if (kb.IsKeyUp(Keys.Escape)) allow_escape = true;
        //    }
        //    return false;
        //}
    }
